...
public class ChangeLog {
	...
	private static final String PREF_FILE = "com.monika.changelog";
	private static final String VERSION_EMPTY = "";
	private static final String VERSION_KEY = "com.monika.changelog.key";
	...
	private ChangeLog(Context c, SharedPreferences sp) {
		...
		try {
			currentVersion = c.getPackageManager().getPackageInfo(c.getPackageName(), 0).versionName;
	...
	public static final String getOldVersion(Context c, String prefFile) {
		String old = c.getSharedPreferences(prefFile, Context.MODE_PRIVATE).getString(VERSION_KEY, VERSION_EMPTY);
		return old;
	...
	public static final String getCurrentVersion(Context c) {
		String current;
		try {
			current = c.getPackageManager().getPackageInfo(c.getPackageName(), 0).versionName;
		} catch (NameNotFoundException e) {
	...
	public AlertDialog getDialog(boolean full) {
		...
		webView.loadDataWithBaseURL(null, getChangeLogFromResources(full), "text/html", "UTF-8", null);
		...
		.setView(webView).setCancelable(false)
