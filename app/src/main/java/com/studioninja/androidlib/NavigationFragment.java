package com.studioninja.androidlib;
...
public abstract class NavigationFragment extends Fragment {
	...
	protected SharedPreferences getPreferences() {
		return getActivity().getSharedPreferences(PREF_FILENAME, Context.MODE_PRIVATE);
	}

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		mUserLearnedDrawer = getPreferences().getBoolean(PREF_KEY_USER_LEARNED_DRAWER, false);
	}
	...
	public void setUp(int fragmentId, DrawerLayout drawerLayout) {
		...
		mDrawerToggle = new ActionBarDrawerToggle(getActivity(), mDrawerLayout, R.drawable.slide_icon, 0, 0) {
			@Override
			...
			public void onDrawerOpened(View drawerView) {
				...
				if (!mUserLearnedDrawer) {
					...
					getPreferences().edit().putBoolean(PREF_KEY_USER_LEARNED_DRAWER, true).commit();
				}
	...
	/**
	 * Lock the drawer into the open state. The user will not be able to slide to close
	 */
	...
	/**
	 * Lock the drawer into the closed state. The user will not be able to slide to open<br>
	...
	/**
	 * 
	 * @return true if the drawer is locked (the user cannot open it with his finger)
	 */
	...
	public int getEdgeSizeDp() {
		try {
			Field dragger = mDrawerLayout.getClass().getDeclaredField("mLeftDragger");
	...
	/**
	 * Recommended edge size in dp so that any user will be easily able to open the drawer
	 */
	public static final int EDGE_SIZE_RECOMMENDED = 40;

	/**
	 * Set the size of the region in which the user can slide his finger to open the drawer<br>
	 ...
	 * 
	 * Note: This uses reflection, if you don't want the app to crash, add this to your proguard-project.txt:<br>
	 * 
	 * <pre>
