...
public class ShareManager {
	...
	private static final String PREF_FILE = "com.monika.androidlib.share";
	private static final String PREF_KEY_COUNT = "com.monika.androidlib.share.count";
	/**
	 * After how many times should the share dialog be shown for the first time?
	 */
	private static final String PREF_KEY_OFFSET = "com.monika.androidlib.share.offset";
	/** After the first time, each how many times should the dialog show? */
	private static final String PREF_KEY_REPEAT = "com.monika.androidlib.share.repeat";
	private static final int OFFSET_DEFAULT = 7;
	private static final int REPEAT_DEFAULT = 5;

	private static final String PREF_KEY_NEVER = "com.monika.androidlib.share.never";
	...
	public static void show(Context c, String text, boolean hasNeverButton) {
		...
		if (!never) {
			if (current == offset || current > offset && ((current - offset) % repeat) == 0) {
	...
	public static AlertDialog.Builder getShareEditDialog(final Context c, final String promoText) {
		return getShareEditDialog(c, promoText, true);
	}

	public static AlertDialog.Builder getShareEditDialog(final Context c, final String promoText, boolean hasNeverButton) {
		...
		ab.setView(v);
		final EditText et = (EditText) v.findViewById(R.id.share_dialog_et_content);
		et.setText(promoText);
		ab.setCancelable(false);
		ab.setTitle(R.string.lib_share_dlg_tit);
		ab.setMessage(R.string.lib_share_dlg_msg);
		if (hasNeverButton) {
			ab.setNegativeButton(R.string.lib_share_never, new OnClickListener() {
		...
		ab.setPositiveButton(android.R.string.ok, new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				Intent sender = Intent.createChooser(intent, c.getString(R.string.lib_share_dlg_tit));
