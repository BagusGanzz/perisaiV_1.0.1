...
import com.studioninja.locker.lock.PasswordView.OnNumberListener;
import com.studioninja.locker.lock.PatternView.Cell;
import com.studioninja.locker.lock.PatternView.DisplayMode;
import com.studioninja.locker.lock.PatternView.OnPatternListener;
import com.studioninja.locker.ui.PreferenceConnector;
...
public class LockService extends Service implements View.OnClickListener, View.OnKeyListener {
	...
	private enum ViewState {
		...
		SHOWING,
		/**
		 * The view has been completely animated and the user is ready to interact with it
		 */
		SHOWN,
		/**
	...
	/**
	 * Create a new password by asking the user to enter it twice (either number or pattern)
	 */
	...
	/**
	 * When the action is {@link #ACTION_COMPARE} use {@link #EXTRA_PACKAGENAME} for specifying the target app.
	 */
	public static final String EXTRA_PACKAGENAME = CLASSNAME + ".extra.target_packagename";
	/**
	 * A {@link LockPreferences} providing additional details on how this {@link LockService} should behave. You should start with a {@link LockPreferences#getDefault(Context)} and change only the
	...
	/**
	 * Time in terms of {@link System#nanoTime()} when the first finger was placed on the view
	 */
	private long mTimeFirstFingerDown;

	/**
	 * Time in terms of {@link System#nanoTime()} when the view was completely shown
	 */
	...
	public static final void hide(Context c) {
		Log.e(TAG, "hide service started");
		Intent i = new Intent(c, LockService.class);
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (ACTION_NOTIFY_PACKAGE_CHANGED.equals(intent.getAction())) {
			...
			mIntent = intent;
			if (Util.checkDrawOverlayPermission(this,0))
			showView();
		}
		return START_NOT_STICKY;
	}
	 ...
	 *
	 * @param explicit
	 *            true if the user has clicked the OK button to explicitly ask for a password check (this should never happen)
	 */
	...
	private void doConfirmPassword() {
		...
		if (!newValue.equals(mNewPassword)) {
			Toast.makeText(this, R.string.password_change_not_match, Toast.LENGTH_SHORT).show();
			setupFirst();
			return;
		}
		PrefUtils prefs = new PrefUtils(this);
		prefs.put(R.string.pref_key_password, newValue);
		prefs.putString(R.string.pref_key_lock_type, R.string.pref_val_lock_type_password);
		prefs.apply();
		Toast.makeText(this, R.string.password_change_saved, Toast.LENGTH_SHORT).show();
		exitCreate();
	}
	...
	private void doConfirmPattern() {
		...
		if (!newValue.equals(mNewPattern)) {
			Toast.makeText(this, R.string.pattern_change_not_match, Toast.LENGTH_SHORT).show();
			mLockPatternView.setDisplayMode(DisplayMode.Wrong);
			setupFirst();
			return;
		}
		...
		prefs.putString(R.string.pref_key_lock_type, R.string.pref_val_lock_type_pattern);
		// Save size as a string
		prefs.put(R.string.pref_key_pattern_size, String.valueOf(options.patternSize));
		prefs.apply();
		Toast.makeText(this, R.string.pattern_change_saved, Toast.LENGTH_SHORT).show();
		exitCreate();
	}
	...
	private void exitSuccessCompare() {
		...
		if (mServiceState == ServiceState.BOUND) {
			...
			if (DEBUG_BIND)
				Log.w(TAG, "Not bound to lockservice (mServiceState=" + mServiceState + ")");
		}
		finish(true);
	}
	...
	private void hideViewAnimate() {
		...
		mAnimHide = AnimationUtils.loadAnimation(this, options.hideAnimationResId);
	...
	private View inflateRootView() {
		...
		mFooterButtons = (LinearLayout) root.findViewById(R.id.lock_footer_buttons);
	...
	private boolean beforeInflate() {
		...
		mLayoutParams = new WindowManager.LayoutParams(WindowManager.LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.TYPE_PRIORITY_PHONE,

		// Whatsapp bug fixed!
				WindowManager.LayoutParams.FLAG_ALT_FOCUSABLE_IM | WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL | WindowManager.LayoutParams.FLAG_FULLSCREEN
	...
	private void setBackground() {
		...
		int theme = PreferenceConnector.readInteger(getApplicationContext(), PreferenceConnector.CURRENT_THEME, 12);
		if (theme == 1) {
//			mViewBackground.setImageResource(R.drawable.theme1_bg);
		} else if (theme == 2) {
			mViewBackground.setImageResource(R.drawable.theme2_bg);
		} else if (theme == 3) {
			mViewBackground.setImageResource(R.drawable.theme3_bg);
		} else if (theme == 4) {
			mViewBackground.setImageResource(R.drawable.theme4_bg);
		} else if (theme == 5) {
			mViewBackground.setImageResource(R.drawable.theme5_bg);
		} else if (theme == 6) {
			mViewBackground.setImageResource(R.drawable.theme6_bg);
		} else if (theme == 7) {
			mViewBackground.setImageResource(R.drawable.theme7_bg);
		} else if (theme == 8) {
			mViewBackground.setImageResource(R.drawable.theme8_bg);
		} else if (theme == 9) {
			mViewBackground.setImageResource(R.drawable.theme9_bg);
		} else if (theme == 10) {
			mViewBackground.setImageResource(R.drawable.theme10_bg);
		} else if (theme == 11) {
			mViewBackground.setImageResource(R.drawable.theme11_bg);
//		} else if (theme == 12) {
//			mViewBackground.setImageResource(R.drawable.theme12_bg);
		}

		// if (blue.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_blue));
		// } else if (dark_blue.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_dark_blue));
		// } else if (green.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_green));
		// } else if (purple.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_purple));
		// } else if (red.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_red));
		// } else if (turquoise.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_turquoise));
		// } else if (orange.equals(options.background)) {
		// mViewBackground.setBackgroundColor(getResources().getColor(R.color.flat_orange));
		...
		// mViewBackground.setImageResource(R.drawable.theme1_bg);
		// // mLockPasswordView.setBackgroundResource(R.drawable.enter_pwd_btn);
		// }
	}
	...
	private final boolean showPasswordView() {
		...
		mTextViewPassword = (TextView) li.inflate(R.layout.view_lock_number_textview, null);
	...
	private final boolean showPatternView() {
		...
		mLockPatternView = (PatternView) mLockView.findViewById(R.id.patternView);
		mLockPatternView.setOnPatternListener(mPatternListener);
		mLockPatternView.setSelectedBitmap(options.patternCircleResId);
		Drawable gd = getResources().getDrawable(R.drawable.passwordview_button_background);
	...
	/**
	 * Runs {@link #beforeInflate()}, inflates the view, adds it to the window, and calls {@link #afterInflate()}<br>
	...
	private void showViewAnimate() {
		...
		mAnimShow = AnimationUtils.loadAnimation(this, options.showAnimationResId);
	 ...
	 * 
	 */
	private static abstract class BaseAnimationListener implements AnimationListener {
	...
	/**
	 * Updates the password, trimming it if necessary, also updates {@link #mTextViewPassword}
	 */
	private void updatePassword() {
		String pwd = mLockPasswordView.getPassword();
		if (MAX_PASSWORD_LENGTH != 0) {
			if (pwd.length() >= MAX_PASSWORD_LENGTH) {
				mLockPasswordView.setPassword(pwd.substring(0, MAX_PASSWORD_LENGTH));
			}
		}
