...
public class PasswordView extends ViewGroup implements OnClickListener, OnLongClickListener {
	...
	private int mOkImageResource = R.drawable.done;
	private int mBackImageResource = R.drawable.x;
	...
	/**
	 * If this is true, all children will be square, so the height of this view will be {@link #mChildHeight} * {@link #mRows} and the width will be {@link #mChildWidth} * {@link #mCols}<br>
	 * This invalidates {@link #mMaxHScale} and {@link #mMaxVScale} <br>
	 * This only works when {@link #mVerticalSpacing} and {@link #mHorizontalSpacing} are the same.
	 */
	...
	public PasswordView(Context context, AttributeSet attrs) {
		...
		try {
			mHorizontalSpacing = a.getDimensionPixelSize(R.styleable.PasswordView_horizontalSpacing, 0);
			mVerticalSpacing = a.getDimensionPixelSize(R.styleable.PasswordView_verticalSpacing, 0);
			...
			mMaxWidth = a.getDimensionPixelSize(R.styleable.PasswordView_maxWidth, 0);
			mMaxHeight = a.getDimensionPixelSize(R.styleable.PasswordView_maxHeight, 0);
			mMaxHScale = a.getFloat(R.styleable.PasswordView_maxHScale, 1F);
			mMaxVScale = a.getFloat(R.styleable.PasswordView_maxVScale, 1F);

			mSquareChildren = a.getBoolean(R.styleable.PasswordView_squareChildren, false);
	...
	protected void onFinishInflate() {
		...
		mButtons = new Button[] { (Button) findViewById(R.id.numlock_b0), (Button) findViewById(R.id.numlock_b1), (Button) findViewById(R.id.numlock_b2), (Button) findViewById(R.id.numlock_b3),
				(Button) findViewById(R.id.numlock_b4), (Button) findViewById(R.id.numlock_b5), (Button) findViewById(R.id.numlock_b6), (Button) findViewById(R.id.numlock_b7),
				(Button) findViewById(R.id.numlock_b8), (Button) findViewById(R.id.numlock_b9) };
		...
		mBackButton.setBackgroundResource(mBackImageResource);
		mBackButton.setOnClickListener(this);
		mBackButton.setOnLongClickListener(this);

		mOkButton.setBackgroundResource(mOkImageResource);
	...
	/**
	 * 
	 * @return The distance in inches that the finger has swiped over the pattern<br>
	 *         This is calculated as the distance between the pattern circles, not the real distance of the finger
	 */
	...
	private void onNumberButtonImpl(View v) {
		...
		Button Num0 = (Button) findViewById(R.id.numlock_b0);
		Button Num1 = (Button) findViewById(R.id.numlock_b1);
		Button Num2 = (Button) findViewById(R.id.numlock_b2);
		Button Num3 = (Button) findViewById(R.id.numlock_b3);
		Button Num4 = (Button) findViewById(R.id.numlock_b4);
		Button Num5 = (Button) findViewById(R.id.numlock_b5);
		Button Num6 = (Button) findViewById(R.id.numlock_b6);
		Button Num7 = (Button) findViewById(R.id.numlock_b7);
		Button Num8 = (Button) findViewById(R.id.numlock_b8);
		Button Num9 = (Button) findViewById(R.id.numlock_b9);
		...
		// final String newPassword = new StringBuilder().append(mPassword).append(b.getText()).toString();
		final String newPassword = new StringBuilder().append(mPassword).append(number).toString();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mWidth = resolveSize(MeasureSpec.getSize(widthMeasureSpec), widthMeasureSpec);
		mHeight = resolveSize(MeasureSpec.getSize(heightMeasureSpec), heightMeasureSpec);
		...
		int childMSW = MeasureSpec.makeMeasureSpec(mChildWidth, MeasureSpec.EXACTLY);
		int childMSH = MeasureSpec.makeMeasureSpec(mChildHeight, MeasureSpec.EXACTLY);
	...
	private void correctViewSize(int width, int height, int maxWidth, int maxHeight, float maxHScale, float maxVScale) {
		...
		mChildWidth = (width - mPaddingLeft - mPaddingRight - horizontalSpacing) / mCols;
		mChildHeight = (height - mPaddingTop - mPaddingBottom - verticalSpacing) / mRows;

		// Set the correct values
		mWidth = mPaddingLeft + mPaddingRight + (mChildWidth * mCols) + (mHorizontalSpacing * (mCols - 1));
		mHeight = mPaddingTop + mPaddingBottom + (mChildHeight * mRows) + (mVerticalSpacing * (mRows - 1));
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			childL = mPaddingLeft + ((mHorizontalSpacing + mChildWidth) * (i % mCols));
			childT = mPaddingTop + ((mVerticalSpacing + mChildHeight) * (i / mCols));

			child.layout(childL, childT, childL + mChildWidth, childT + mChildHeight);
		}
	}
	...
	/**
	 * Update the internal password this {@link PasswordView} is working with. If password is null then it will be cleared.
	 * 
	 * @param password
	 */
