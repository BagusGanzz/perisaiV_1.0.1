...
public class ProItemView extends LinearLayout {
	...
	public ProItemView(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ProItemView);
		try {
	...
	public void setSelectedLockType(boolean selected) {
		if (selected) {
			...
			mTitleView.setTextColor(getResources().getColor(R.color.pro_item_selected_tit));
			mTitleView.setTypeface(null, Typeface.BOLD);
			mDescriptionView.setTextColor(getResources().getColor(R.color.pro_item_selected_desc));
		} else {
			setBackgroundResource(R.drawable.item_not_selected);
			mTitleView.setTypeface(null, Typeface.NORMAL);
			mTitleView.setTextColor(getResources().getColor(R.color.pro_item_tit));
			mDescriptionView.setTextColor(getResources().getColor(R.color.pro_item_desc));
		}
	}
}
