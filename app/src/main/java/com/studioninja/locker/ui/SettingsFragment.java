...
import com.studioninja.util.ChangeLog;
import com.studioninja.locker.R;

@SuppressLint("NewApi")
public class SettingsFragment extends PreferenceFragment implements OnPreferenceChangeListener, OnSharedPreferenceChangeListener,
...
//		mBackground.setOnPreferenceChangeListener(this);
		...
		if (code != null)
			mRecoveryPref.setSummary(String.format(getString(R.string.pref_desc_recovery_code), code));
	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		...
		if (key.equals(keyDelayTime)) {
			...
			if (isZero) {
				...
				mEditor.putBoolean(getString(R.string.pref_key_delay_status), false);
				mEditor.commit();

			} else {
				String res = String.valueOf(Long.parseLong(newTime));
				if (!newTime.equals(res)) {
					mEditor.putString(getString(R.string.pref_key_delay_status), res);
					mEditor.commit();
				}
			}
			...
			try {
				...
				newPatternWidth = Integer.parseInt(getActivity().getString(R.string.pref_def_pattern_size));
			}
			LockService.showCreate(getActivity(), LockPreferences.TYPE_PATTERN, newPatternWidth);
			return false;
		}
		return true;
	}
	...
	private void backgroundFromGallery() {
		...
		List<ResolveInfo> ris = getActivity().getPackageManager().queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
		if (ris.size() > 0) {
	...
	public void onActivityResult(int req, int res, Intent data) {
		...
		Toast.makeText(getActivity(), R.string.background_changed, Toast.LENGTH_SHORT).show();
	...
	public static AlertDialog getChangePasswordDialog(final Context c) {
		...
		choose.setItems(R.array.lock_type_names, new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				int lockType = which == 0 ? LockPreferences.TYPE_PASSWORD : LockPreferences.TYPE_PATTERN;
