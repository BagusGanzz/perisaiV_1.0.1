...
public class Hide_photos extends Activity implements OnItemClickListener, OnClickListener {
	...
	private class MovefileTask extends AsyncTask<Context, Void, String> {
		...
		private void MoveFile(String path) {
			...
			String pathn = path.substring(path.indexOf(".Photo/") + 6);
			...
			for (int i = 0; i < separated.length - 1; i++) {
				...
				if (i == 0) {
					...
					if (newpath.equals("")) {
						...
						if ((separated[i].equals(".Android_Libraries")) || (separated[i].equals(".Photo"))) {
			...
			try {
				...
				// we will copy the file
					...
					if (sourceLocation.exists()) {
						...
						values.put(MediaStore.Images.Media.DATA, targetLocation.getAbsolutePath());
						values.put(MediaStore.Images.Media.DATE_TAKEN, targetLocation.lastModified());

						Uri mImageCaptureUri = Hide_photos.this.getContentResolver().insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values);

						// to notify change
						Hide_photos.this.getContentResolver().notifyChange(Uri.parse("file://" + targetLocation.getAbsolutePath()), null);
		...
		protected void onPostExecute(String result) {
			...
			if (Index < Album_id.size()) {
				...
				setup();
				Toast.makeText(getApplicationContext(), "Photo Has Been Restore", Toast.LENGTH_SHORT).show();
			}
