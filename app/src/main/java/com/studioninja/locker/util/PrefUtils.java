...
import com.studioninja.locker.lock.LockPreferences;
...
public class PrefUtils {
	public static final String PREF_FILE_DEFAULT = "com.monika.locker.prefs.default";
	private static final String PREF_FILE_APPS = "com.monika.locker.prefs.apps";
	private static final String ALIAS_CLASSNAME = "com.monika.locker.MainActivityAlias";
	...
	public PrefUtils(Context c) {
		mContext = c;
		mPrefs = mContext.getSharedPreferences(PREF_FILE_DEFAULT, Context.MODE_PRIVATE);
	}
	...
	public SharedPreferences apps() {
		if (mApps == null) {
			mApps = mContext.getSharedPreferences(PREF_FILE_APPS, Context.MODE_PRIVATE);
		}
		return mApps;
	}
	...
	public Editor putString(int keyResId, int valueResId) {
		final Editor editor = editor();
		editor.putString(mContext.getString(keyResId), mContext.getString(valueResId));
		return editor;
	}
	...
	public String getString(int keyResId, int defResId) {
		final String key = mContext.getString(keyResId);
		return (mPrefs.contains(key)) ? mPrefs.getString(key, null) : mContext.getString(defResId);
	}
	...
	public Integer parseInt(int keyResId, int defResId) {
		...
		return (result != null) ? result : Integer.parseInt(mContext.getString(defResId));
	}
	...
	public Long parseLong(int keyResId, int defResId) {
		final Integer result = parseInt(keyResId);
		return (result != null) ? result : Long.parseLong(mContext.getString(defResId));
	}
	...
	public boolean getBoolean(int keyResId, int defResId) {
		...
		return result != null ? result : mContext.getResources().getBoolean(defResId);
	}
	...
	public int getCurrentLockTypeInt() {
		...
		if (type.equals(mContext.getString(R.string.pref_val_lock_type_password)))
			return LockPreferences.TYPE_PASSWORD;
		else if (type.equals(mContext.getString(R.string.pref_val_lock_type_pattern)))
	...
	public static void setHideApplication(Context c, boolean hide) {
		...
		int setting = hide ? PackageManager.COMPONENT_ENABLED_STATE_DISABLED : PackageManager.COMPONENT_ENABLED_STATE_ENABLED;
